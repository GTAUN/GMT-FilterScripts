/*
				#############################################################################
				##                                                                         ##
				##                                 tjjjjjj   jjjjjjjjjjjjjjt     jjjjjjjjjj##
				##                               jjjjjjjjj   jjjjjjjjjjjjjjjjt   jjjjjjjjjj##
				##             jj;              jjjt         jj      jj    ijj.      jj    ##
				##            jjjj             jjj           jj      jj     jjj      jj    ##
				##           tjj jj            jj            jj      jj     jjj      jj    ##
				##           jj  tji           jj            jj      jj     jjj      jj    ##
				##          jji   jj           jj            jj      jj     jjj      jj    ##
				##          jj    ;jj          ff            ff      ff     jff      ff    ##
				##         jj:     jj          ff      fff   ff      ff     jff      ff    ##
				##        ,jj  jj   jj         ff      fff   ff      ff     jff      ff    ##
				##        jj   i:   jj         ff      fff   ff      ff     jff      ff    ##
				##       jfj  iff;   ff        ff      fff   ff      ff     fff      ff    ##
				##       ff  fjffif  if:       ff      fff   ff      ff     fff      ff    ##
				##      jfi  f ff  fj ff       fff     fff   ff      ff     fff      ff    ##
				##      ff   i ff     fft      :LLLLLLLLLL   LL      LL     fLL      LL    ##
				##     ff      ff      ff        ,LLLLLLLL   LL      LL     fLL      LL    ##
				##    tff     ji f      ff                                                 ##
				##    ff:     f  f      ff:                                                ##
				##   fff          f      ff         LLLL                     L             ##
				##   ff                  fft        L  L   L             L                 ##
				##  fff   j;i;; ;i;i;i    ff        LL    LLLL  L  L  LLLL  LL    LLL      ##
				##  jLLLLLLLLLLLLLLLLLLLLLLL         LL    L    L  L LL LL   L   L  L      ##
				##   LLLLLLLLLLLLLLLLLLLLLLL          LL   L    L  L LL  L   L   L  L      ##
				##                                  L  L   L L  L LL  L LL   L   L  L      ##
				##                                  LLLL   LLL  LL L  LLLL  LLL   LLL      ##
				#############################################################################
				
				[INFO]            汉化信息

				[OBJECT]          汉化工程名称   : xVIP
				{OBJECT CODE]     工程代号       : x-Fs201301
				[TRANSLATOR]      工程小组       : GMT
				[ORIGINAL AUTHOR] 原作者         : XtremeR

                 ●本汉化资源是经由原作者同意并授权制作,未经许可,不允许任何修改原作者以及汉化的版权信息的行为.

                 ●双重许可证保护 : GPL 及 MIT (https://github.com/GTAUN/GMT-FilterScripts/blob/master/LICENSE.md) 

                 ●违规操作者将以《中华人民共和国知识产权法》以及美国《版权法》[Copyright law of the United States],我们将保留法律诉讼的权利.
                 
                 ●如有任何问题 欢迎发送邮件到  Yinjin_116s@163.com
*/
...
#undef  					MAX_PLAYERS
#define 					MAX_PLAYERS 		32                    			// 此处设置服务器允许最大玩家数 [服务端 server.cfg]
...
#define 					PATH 				"/Users/%s.ini"  				//此处设置玩家数据保存路径
...
{
    new string[200];
	format(string, sizeof(string), " 欢迎回来 %s  您当前的VIP等级为: %d", PlayerName(playerid), Vip[playerid] );
	SendClientMessage(playerid ,COLOR_RED, string);
    tune[playerid] = 0;
    asked1[playerid] = 0;
	asked[playerid] = 0;
	ship[playerid] = 0;
	SendClientMessage(playerid,-1,"xVIP FS By XtremeR");						//原作者信息禁止修改
...
public OnPlayerSpawn(playerid)
{
	...
	if(Vip[playerid] >= 1) 
	{
		VIPS[playerid] = Create3DTextLabel(" VIP 玩家", COLOR_VIP, 0.0, 0.0, 0.0, 50.0, 0, 0);
...
{
    if(newstate == PLAYER_STATE_DRIVER || newstate == PLAYER_STATE_PASSENGER)// 本代码将监视镜头转进被监视玩家所进入的载具 If the player's state changes to a vehicle state we'll have to spec the vehicle.
    {
        if(IsBeingSpeced[playerid] == 1)//检测是否被监视		If the player being spectated, enters a vehicle, then let the spectator spectate the vehicle.
        {
            foreach(Player,i)
            {
                if(spectatorid[i] == playerid)
                {
                    PlayerSpectateVehicle(i, GetPlayerVehicleID(playerid));//将监视模式设置为目标所在载具 Letting the spectator, spectate the vehicle of the player being spectated (I hope you understand this xD)
                }
            }
        }
    }
    ...
    {
        if(IsBeingSpeced[playerid] == 1)//检测是否被监视 If the player being spectated, exists a vehicle, then let the spectator spectate the player.
        {
            foreach(Player,i)
            {
                if(spectatorid[i] == playerid)
                {
                    PlayerSpectatePlayer(i, playerid);//将监视模式设置为目标 Letting the spectator, spectate the player who exited the vehicle.
                }
            }
        }
    }
...
public OnPlayerInteriorChange(playerid, newinteriorid, oldinteriorid)//本回调将检测玩家内部空间的动态 This is called when a player's interior is changed.
{
    if(IsBeingSpeced[playerid] == 1)//如果玩家动态改变 我们将更新监视模式所在的空间 If the player being spectated, changes an interior, then update the interior and virtualword for the spectator.
    {
        foreach(Player,i)
        {
            if(spectatorid[i] == playerid)
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    			switch(dialogid)
    			{
					case 245:
					{
						if(!response)
						{
							SendClientMessage(playerid, COLOR_RED,"取消动作!");
							return 1;
						}
						switch(listitem)
						{
						...
						switch(listitem)
						{
							case 0:
							{
								if (Vip[playerid] >= 1)
								{
									if (de[playerid] == 0)
									{
										if(IsPlayerInAnyVehicle(playerid)) return SendClientMessage(playerid, COLOR_RED,"你已经有辆载具了!");
									...
									else return SendClientMessage(playerid,COLOR_RED,"ERROR: 本指令只能在每次复活后使用1次");
								} 
								else return SendClientMessage(playerid,COLOR_RED,"ERROR: VIP等级1或以上方可使用本指令");
							}
							case 1:
							{
								if (Vip[playerid] >= 1)
								{
									SendClientMessage(playerid,COLOR_RED,"加装氮气完毕");
							...
							case 2:
							{
								if (Vip[playerid] >= 1)
								{
									SendClientMessage(playerid,COLOR_RED,"***成功设置专属颜色!***");
							...
							case 4:
							{
								if (Vip[playerid] >= 1)
								{
									...
									{
										SendClientMessage(playerid,COLOR_RED,"恢复HP!");
										SetPlayerHealth(playerid,100);
							...
							case 5:
							{
								if (Vip[playerid] >= 1)
								{
									if(Anti_heal[playerid] == 0) 
									{
										SendClientMessage(playerid,COLOR_RED,"恢复AP!");
										SetPlayerArmour(playerid,100);
										Anti_heal[playerid] = 1;
									} else return SendClientMessage(playerid,COLOR_RED,"ERROR: 本指令只能在每次复活后使用1次");
								} else return SendClientMessage(playerid,COLOR_RED,"ERROR: VIP等级1或以上方可使用本指令");
							...
							case 6:
							{
								ShowPlayerDialog(playerid,245,DIALOG_STYLE_LIST,"颜色列表","{FCF7F9}白\n{1C9139}绿\n{2D5CAD}蓝\n{E8B82A}橙\n{5C512F}棕\n{16F2E7}天蓝\n{FF0000}红\n{FF42EF}粉\n{B907F5}紫 \n{878478}灰\n{000000}黑","确定","取消");
							}
							case 7:
							{
								if (Vip[playerid] >= 1)
								{
									SendClientMessage(playerid,COLOR_RED,"无敌车来了!");
									SetVehicleHealth(playerid,99999999999999999.0);
								} 
								else 
								{
									SendClientMessage(playerid,COLOR_RED,"ERROR: 指令无效");
								}
							}
						}
					}
...
stock xVip(playerid,command[])
{
	...
	format(string,sizeof(string),"[INFO]V.I.P %s 使用专属功能%s",string,command);
	MessageToD(blue,string);
}

CMD:vips(playerid, params[])//查看在线大爷
{
    ...
    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        if (IsPlayerConnected(i))
        {
            if (Vip[playerid] >= 1)
            {
                format(string, 500, "%s %s [ID:%i] | VIP 等级: %d\n", string, PlayerName(i), i, Vip[playerid]);
                count++;
            }
        }
	}
    if (count == 0) ShowPlayerDialog(playerid, 800, DIALOG_STYLE_MSGBOX, "{F81414}在线 VIP 名单", "{00FFEE}鬼影都没一个", "关闭", "");
    else ShowPlayerDialog(playerid, 800, DIALOG_STYLE_MSGBOX, "{F81414}在线 VIP 名单", string, "关闭", "");
    return 1;
}


CMD:vannounce(playerid,params[])//发公告
{
    if (Vip[playerid] >= 3)
		{
    	if(isnull(params)) return SendClientMessage(playerid,red,"使用指南: /announce <在这里公布你的话给全世界>");
    	xVip(playerid,"VANNOUNCE");
		return GameTextForAll(params,4000,3);
}
    return 1;
}
CMD:vdick( playerid, params[] )//汉化者语:巨恶心 - - How CAN YOU DO THIS =A=...
{
	if (Vip[playerid] >= 2)
	{
		...
		SendClientMessage(playerid, -1,"鸡鸡出现了 ");
		xVip(playerid,"VDICK");
	}
	return 1;
}
	
CMD:vgod( playerid, params[] )//上帝模式
{
	if (Vip[playerid] >= 2)
	{
...
CMD:vjetpack( playerid, params[] )//火箭飞行包
{
	if (Vip[playerid] >= 2)
	{
...
CMD:vspec(playerid, params[])//监视模式
{
    if (Vip[playerid] >= 2)
	{
		new id;
		if(!IsPlayerAdmin(playerid))return 0;
		if(sscanf(params,"u", id))return SendClientMessage(playerid, Grey, "使用指南: /spec [需要观察的id]");
		if(id == playerid)return SendClientMessage(playerid,Grey,"无法观察自己.");
		if(id == INVALID_PLAYER_ID)return SendClientMessage(playerid, Grey, "未找到指定玩家!");
		if(IsSpecing[playerid] == 1)return SendClientMessage(playerid,Grey,"你已进入监视模式,无法操作.");
		...
		format(String, sizeof(String),"你已开始监视玩家 %s.",Name);
...
CMD:ballooncmds(playerid, params[])//放气球
{
 	#define DIALOG_B 2
    if (Vip[playerid] >= 1)
	{
	    ShowPlayerDialog(playerid, DIALOG_B, DIALOG_STYLE_MSGBOX, "气球", 
		"\
		/rballon 红气球\n/bballon 蓝气球\n/gballon 灰气球\n/rup - 红气球上移, /rdown - 红气球下移\n\
		/bup - 蓝气球上移 /bdown - 蓝色气球下移\n/gup - 灰气球上移, /gdown - 灰气球下移", "关闭", "");
	}
	return 1;
}
CMD:rballoon(playerid, params[])
{
    if (Vip[playerid] >= 1)
	{
		...
		SendClientMessage(playerid, 0xDEEE20FF, "欢迎选择红色气球");
...
CMD:bballoon(playerid, params[])
{
    if (Vip[playerid] >= 1)
	{
		...
		SendClientMessage(playerid, 0xDEEE20FF, "欢迎选择蓝色气球");
...
CMD:gballoon(playerid, params[])
{
    if (Vip[playerid] >= 1)
	{
		...
		SendClientMessage(playerid, 0xDEEE20FF, "欢迎选择灰色气球");
...
CMD:rup(playerid, params[])
{
    if (Vip[playerid] >= 1)
	{
		...
		SendClientMessage(playerid, 0xDEEE20FF, "我顶!");
		xVip(playerid,"RUP");
	}
	return 1;
}
...
CMD:rdown(playerid, params[])
{
    if (Vip[playerid] >= 1)
	{
		...
		SendClientMessage(playerid, 0xDEEE20FF, "我降!");
		xVip(playerid,"RDOWN");
	}
	return 1;
}
...
CMD:vspecoff(playerid, params[])
{
    if (Vip[playerid] >= 2)
	{
		if(IsSpecing[playerid] == 0)return SendClientMessage(playerid,Grey,"你不在监视模式里.");
...
CMD:vloadpos(playerid, params[])
{
	if (Vip[playerid] >= 2)
	{
		...
		else return SendClientMessage(playerid, 0xFFFFFFFF, "系统没有上次坐标的记录!");
	}
    return 1;
}
...
CMD:vsavepos(playerid, params[])
{
	if (Vip[playerid] >= 2)
	{
		...
		SendClientMessage(playerid, 0xFFFFFFFF, "成功记录位置！ 你可以输入 /vloadpos 传送到记录的位置!");
...
CMD:untune(playerid, params[])
{
	if (Vip[playerid] >= 1)
	{
		if(tune[playerid] == 1)
		{
			...
			SendClientMessage(playerid,-1,"SERVER:清除改装完毕 !");
		}
		else
		if(tune[playerid] == 0)
		{
			SendClientMessage(playerid,-1,"SERVER:你的载具没被改装!");
		}
	}
	return 1;
}
...
CMD:vtune(playerid, params[])
{
	if (Vip[playerid] >= 1)
	{
		...
		if(GetVehicleModel(vehicleid) == 411) // 411 is the infernus model
		{
			...
			xVip(playerid,"VTUNE");
			SendClientMessage(playerid, 0xDEEE20FF, "成功改装Infernus!");
		}
		else
		if(GetVehicleModel(vehicleid) == 541) // 541 is the Bullet model
		{
			...
			ChangeVehicleColor(vehicleid,0,0);
			xVip(playerid,"VTUNE");
			SendClientMessage(playerid, 0xDEEE20FF, "成功改装Bullet!");
		}
		else
		{
			tune[playerid] = 0;
			SendClientMessage(playerid,-1,"SERVER:改装车仅限于 Inferous 和 Bullet之间选择.");
		}
	}
	return 1;
}
CMD:vtunecmds(playerid, params[])
{
	...
	if (Vip[playerid] >= 1)
	{
		ShowPlayerDialog(playerid, tu, DIALOG_STYLE_MSGBOX, "载具改装指令", "/vtune - 改装你的车!!\n/untune - 解除改装\n\n\n注意: 只允许改装 INFEROUS以及BULLET", "关闭", "");
    }
    return 1;
}
CMD:visland(playerid, params[])
{
	if (Vip[playerid] >= 2)
	{
		...
		SendClientMessage(playerid, COLOR_YELLOW,"欢迎进入VIP专属岛屿");
    }
    return 1;
}
//=========================================
...
CMD:piratecmds(playerid, params[])
{
	...
	if (Vip[playerid] >= 1)
	{
		ShowPlayerDialog(playerid, DIALOG_P, DIALOG_STYLE_MSGBOX, "海盗船驾驶指南!", "/VPirate - 刷出一艘海盗船\n/hidebox - 隐藏专属光环 (聊天泡泡)\n/yes - 确定驾驶海盗船!!\n/no - 不使用海盗船!", "Close", "");
    }
    return 1;
}
...
CMD:vpirate(playerid, params[])
{
	if (Vip[playerid] >= 1)
	{
		...
		SendClientMessage(playerid, 0xFF0080C8, "海盗船长 一路顺风!");//spawning that BOAT :D
...
CMD:vcmds(playerid, params[])
{
	if (Vip[playerid] >= 1)
	{
		new XVip1[]="{FF4101}Lv 1 V.I.P 指令列表\n\
		/vcar       /vbike     \n\
		/vheli      /vboat     \n\
		/vplane     /vheal     \n\
		/varmour    /balloncmds\n\
		...
		XVip3[]="{2CF11A}Lv 3 V.I.P 指令列表\n\
		/vannounce   /vshop      \n\
		/vheli      /vfeature    \n\
		/vplane     /vironman    \n\
		/varmour    /vgodcar     \n\
		/vgoto\n\n\n\
		{FFFFFF}使用 /vc 进行VIP聊天\n",

		XVcmds[1600];
		format( XVcmds, sizeof XVcmds, "%s%s%s%s%s", XVip1, XVip2, XVip3);
		Vcmds(playerid, V.I.PCmds, xBox, "V.I.P 指令列表", XVcmds, "Ok", "" );
	}
	return 1;
}
...
CMD:hidebox(playerid, params[])
{
    if (Vip[playerid] >= 1)
	{
		...
		SendClientMessage(playerid, 0xFF0080C8, "隐藏头顶光环!");
    }
    return 1;
}
...
CMD:yes(playerid, params[])
{
	...
	{
		SendClientMessage(playerid,-1,"没人问你问题啊");
	}
    return 1;
}
...
CMD:no(playerid, params[])
{
	if (Vip[playerid] >= 1)
	{
		if (asked1[playerid] == 1)
		{
			...
			asked1[playerid] = 0;
			xVip(playerid,"NO");
			SendClientMessage(playerid, 0xFF0080C8, "想再玩海盗船就输入 /vPirate 吧!");
		}
    }
...
CMD:setvip(playerid, params[])
{
	if (IsPlayerAdmin(playerid) )
	{
		...
		if(!params[0]||!(pos=chrfind(' ',params)+1)||!params[pos]) return SendClientMessage(playerid, COLOR_RED, "[!] 使用指南: /setlevel [ID] [0-3]");
		new id = strval(params[0]);
		level = strval(params[pos]);
		if(!IsPlayerConnected(id))  return SendClientMessage(playerid, COLOR_RED, "  [!] 指定ID的玩家不在服务器!");
		if(level < 0 || level > 3) return SendClientMessage(playerid, COLOR_RED, "  [!] 选定等级需要在 0 和 3 之间选择 !");
		format(string, sizeof(string), " **管理员 %s 设置 VIP称号予 %s [等级: %d]", PlayerName(playerid), PlayerName(id), level);
...
CMD:unship(playerid, params[])
{
	if(ship[playerid] == 1)
	{
		RemovePlayerFromVehicle(playerid);
		DestroyObject(o);
		SendClientMessage(playerid,-1,"SERVER:海盗船清理完毕! !");
		xVip(playerid,"UNSHIP");
	}
	else
	if(ship[playerid] == 0)
	{
		SendClientMessage(playerid,-1,"SERVER:你都没有海盗船!");
	}
	return 1;
	}
...
{
new pos, level;
if(isnull(params)) return SendClientMessage(playerid, COLOR_RED, "x使用指南: /weahter [weather id]");
level = strval(params[pos]);
if(level < 1 || level > 44) return SendClientMessage(playerid, COLOR_RED, "  请输入一个适当的天气数值 [1-44]");
...
{
	SendClientMessage(playerid,COLOR_RED,"鸣谢 XtremeR 创建 VIP 脚本");
	SendClientMessage(playerid,COLOR_RED,"鸣谢 Tannz0rz 创建 /ironman");
	SendClientMessage(playerid,COLOR_RED,"鸣谢 Speederx 创建 VIP 专属岛屿");
...
CMD:vsay(playerid, params[])
{
	if (Vip[playerid] >= 2)
	{
		...
		if(isnull(params)) return SendClientMessage(playerid, red, "ERROR: 请输入 /vsay [你要说的话]");
		format(string, sizeof(string), "|**~~V.I.P %s ~~**| %s", name, params[0] );
		return SendClientMessageToAll(0xFF9900AA,string);
	} else return SendClientMessage(playerid,red,"ERROR: 权限不足");
}
//Donor Chat!!!
...
CMD:vc(playerid, params[])
{
	if (Vip[playerid] >= 1)
	{
		new name[MAX_PLAYER_NAME+1], string[24+MAX_PLAYER_NAME+1];
		GetPlayerName(playerid, name, sizeof(name));
		if(isnull(params)) return SendClientMessage(playerid, red, "ERROR: 请输入 /vc [你要说的话]");
		format(string, sizeof(string), "~VIP~聊天: %s: %s", name, params[0] );
...
CMD:vheli(playerid, params[])
{
   	if (Vip[playerid] >= 1)
   	{
		if(IsPlayerInAnyVehicle(playerid)) return SendClientMessage(playerid, COLOR_RED,"你已经有一个载具了!");
		new Float:X, Float:Y, Float:Z;
		GetPlayerPos(playerid, X, Y, Z);
		PutPlayerInVehicle(playerid, CreateVehicle(487, X, Y, Z, 0.0,0, 1, 60), 0);
		SendClientMessage(playerid, COLOR_RED,"游戏愉快!");
		xVip(playerid,"VHELI");
 	} else return SendClientMessage(playerid,COLOR_RED,"ERROR: VIP等级1或以上方可使用本指令");
   	return 1;
}
...
CMD:vgoto(playerid, params[])
{
	if (Vip[playerid] >= 3)
	{
		if(isnull(params)) return SendClientMessage(playerid, COLOR_RED, "x使用指南: /vgoto [玩家id]");
		new id = strval(params[0]);
		new Float:x,Float:y,Float:z;
		GetPlayerPos(id,x,y,z);
		SetPlayerPos(playerid,x,y,z);
		xVip(playerid,"VGOTO");
		SendClientMessage(playerid, COLOR_WHITE,"嗖.");
	}
	return 1;
}
...
CMD:vstats(playerid, params[])
{
	#pragma unused params
	new string[200];
	format(string, sizeof(string), " 昵称: %s  Vip 等级: %d", PlayerName(playerid), Vip[playerid] );
	SendClientMessage(playerid ,COLOR_RED, string);
	return 1;
}
...
CMD:vshop(playerid, params[])
{
	...
	{
		xVip(playerid,"VSHOP");
		ShowPlayerDialog(playerid,99,DIALOG_STYLE_LIST,"{00FF00}VIP 玩家商店","\n{FFFFFF}满血和护甲- {00FF00}3000$\n{FFFFFF}RPG(火箭弹)- {00FF00}7800$\n{FFFFFF}手榴弹- {00FF00}6500$","购买","取消");
	}
	return 1;
}
...
CMD:vfeatures(playerid, params[])
{
	if (Vip[playerid] >= 3)
	{
		...
		ShowPlayerDialog(playerid, 786, DIALOG_STYLE_LIST, "xV.I.P 功能","专属汽车(/vcar)\n专属氮气(/vnos)\n专属颜色(/vcolor)\n专属武器(/vweap)\n复活恢复血量(vheal)\n复活恢复AP(/varmour)\n专属汽车颜色(/vcarcolor)\n专属无敌汽车(/vcargod)","选择","关闭");
	}
	return 1;
}
...
CMD:vskin(playerid, params[])
{
	...
	{
		new pos, level;
		if(isnull(params)) return SendClientMessage(playerid, COLOR_RED, "使用指南: /vskin [皮肤 id]");
		level = strval(params[pos]);
		if(level < 1 || level > 299) return SendClientMessage(playerid, COLOR_RED, "ERROR: 请输入一个合适的数值 [1-299]");
		xVip(playerid,"VSKIN");
...
CMD:vkick(playerid, params[])
{
	...
	{
		new id, reason[128];
		if(sscanf(params, "us(No Reason)[128]", id, reason))SendClientMessage(playerid, COLOR_WHITE, "使用指南: /kick [id/name][理由]");
		else if(id==playerid)SendClientMessage(playerid,COLOR_WHITE,"Error: 自己踢自己,你够得着么!");
		else if (id==INVALID_PLAYER_ID)SendClientMessage(playerid,COLOR_WHITE,"Error: 无法找到指定玩家!");
		else 
		{
			new KickMessage[128];
			...
			format(KickMessage, sizeof(KickMessage), "VIP 玩家%s(%d) 踢出玩家 %s(%d). 理由: %s", Name, playerid, Name2, id);
